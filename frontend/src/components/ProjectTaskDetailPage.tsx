...
interface ProjectTaskDetailPageProps {
  task: ProjectTask;
  onBack: () => void;
  onSave: (id: number, patch: Partial<ProjectTask>) => Promise<void>;
...
const STATUS_COLORS: Record<string, string> = {
  'To Do': '#3b82f6', Doing: '#eab308', Completed: '#22c55e', 'On Hold': '#ef4444',
};
...
export function ProjectTaskDetailPage({ task, onBack, onSave, onDelete, userName }: ProjectTaskDetailPageProps) {
    ...
    <div className="tdp">
      <div className="tdp-content">
        ...
        <div className="tdp-toolbar">
          <button className="tdp-tb-btn" onClick={onBack} title="Back to project">
            ...
            <div className="tdp-prop tdp-prop-rel" ref={membersRef}>
              ...
              {membersOpen && (
                <div className="tdp-dropdown tdp-dd-wide">
                  ...
                  {ALL_MEMBERS.map((m) => (
                    <button key={m} className={`tdp-dd-item ${task.assignee === m ? 'sel' : ''}`} onClick={() => { onSave(task.id, { assignee: m }); setMembersOpen(false); }}>
